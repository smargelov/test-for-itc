$(document).ready(function () {
    ...
    let slider = document.getElementById('price-slider'),
        ...
        prices = [5990, 3990, 3400, 5990, 5990, 5990, 3400, 3990],
        ...
        setNewStore = (newStore) => {
            ...
            for (let i = 0; i < cards.length; i++) {
                ...
                if (newCard) {
                    ...
                    $(card).find('.product-card__price').attr('data-price', newCard.price);
            ...
            $.ajax({
                ...
                success: function (response) {
                    response.forEach(element => {
                        cartSumm += Number(element.price);
                        $(`button[data-product='${element.prodId}'`).css('backgroundColor', '#00ae68');
                        if (cartSumm > 0) {
                            $('.header__meta-title > span').text(formatNumber(cartSumm) + ' ₽')
                        } else {
                            $('.header__meta-title > span').text('0')
                        }
                    });
                }
            });
    ...
    $('.product-card__btn').click(function (e) {
        ...
        let th = $(this),
            ...
            productPrice = $(this).siblings('.product-card__price').data('price'),
            result = {
                inFav: false,
                status: '',
                message: ''
        ...
        $.ajax({
            type: "POST",
            url: "http://localhost:3002/cart",
            data: `prodId=${productId}&price=${productPrice}`,
            dataType: "json",
            success: (response) => {
                result.inFav = true;
                result.status = 'SUCCESS';
                result.message = 'Товар успешно добавлен в Корзину';
                $(th).css('backgroundColor', '#00ae68');
                refreshCart();
